...
export default class Parser {
	...
	parse_var_declaration(expectSemicolon: boolean = true): Stmt {
		...
		if (this.at().type == TokenType.Semicolon && expectSemicolon) {
	...
	private parse_object_expr(): Expr {
		...
		if (this.at().type !== TokenType.OpenBrace) {
			return this.parse_relational_expr();
		}
	...
	private parse_for_statement(): Stmt {
		...
		if (this.at().type !== TokenType.Semicolon) {
			if (this.at().type === TokenType.Let || this.at().type === TokenType.Const) {
				// Parse variable declaration, but do NOT expect a semicolon after it
				init = this.parse_var_declaration(false);
			} else {
		...
		this.expect(TokenType.Semicolon, "Expected ; after for test");
